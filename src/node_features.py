...
def load_feature_mapping(path: Path, node_ids: List[str], fmt: str = "auto") -> Tuple[Dict[str, np.ndarray], Dict[str, object]]:
    ...
    if fmt in {"auto", "matrix-id-first", "matrix-node-order", "matrix-row-index0", "matrix-row-index1"}:
        raw = pd.read_csv(path, header=None)
        if fmt in {"auto", "matrix-id-first"} and raw.shape[1] > 1:
            values = raw.iloc[:, 1:].apply(pd.to_numeric, errors="coerce").fillna(0.0).to_numpy(dtype=np.float32)
            raw_ids = raw.iloc[:, 0].astype(str).str.strip().tolist()
            by_id = {raw_ids[i]: values[i] for i in range(len(raw_ids))}
            add_candidate("matrix-id-first", by_id)
            if raw.shape[0] == len(node_ids):
                add_candidate("matrix-id-first-node-order", {node_id: values[i] for i, node_id in enumerate(node_ids)})
    ...
    if fmt != "auto":
        matching = [candidate for candidate in candidates if candidate[0] == fmt]
        if fmt == "matrix-id-first":
            matching = [candidate for candidate in candidates if candidate[0] in {"matrix-id-first", "matrix-id-first-node-order"}]
        if matching:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--drug-feature-format", default="auto", choices=["auto", "id-wide", "id-vector", "matrix-id-first", "matrix-node-order", "matrix-row-index0", "matrix-row-index1"])
    p.add_argument("--disease-feature-format", default="auto", choices=["auto", "id-wide", "id-vector", "matrix-id-first", "matrix-node-order", "matrix-row-index0", "matrix-row-index1"])
    p.add_argument("--protein-feature-format", default="auto", choices=["auto", "id-wide", "id-vector", "matrix-id-first", "matrix-node-order", "matrix-row-index0", "matrix-row-index1"])
